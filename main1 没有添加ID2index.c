...
int userLetter(const int user_index)
{
    ...
    else if(key=='9') {if(delUser(user_index)) getchar(); main();return 1;}
...
void delUserAdm()
{
    ...
    if(key=='y'||key=='Y')
    {
        ...
        if(key=='Y'||key=='y')
        {
            for(int i=0;i<MAX_BOOKS_NUM;i++)
            {
                if(books[i].isLend)
                {
                books[i].lenduserid=0;//清空借书用户ID
                strcpy(books[i].lendMessage,"");//清空备注
                }
            }
    ...
    if(key=='y'||key=='Y')
    {
            ...
            if(users[id].lendBooksNum)//用户有借书未还
            {
                for(int j=0;j<10;j++)
                {
                    int bookindex=searchBook(users[id].lendBooksIsbn[j],1);//查找书籍
                    if(bookindex!=-1)
                    {
                        books[bookindex].isLend=0;
                        books[bookindex].lenduserid=0;;
                        strcpy(books[bookindex].lendMessage,"");
                    }
                }
            }
            users[id].lendBooksNum=0;//借阅的书籍清空
            // memset(&users[id],0,sizeof(struct users));
            // memset(users[id].lendBooksIsbn,0,sizeof(users[id].lendBooksIsbn));
            //清空struct users[id]
            memset(&users[id],0,sizeof(struct Users));
            saveUsers();//保存
            GREEN;
    ...
    for(int i=0;i<MAX_BOOKS_NUM;i++)
    {
        if(strcmp(books[i].isbn,isbn)==0)//如果找到了书籍
        {
            ...
            else //如果书籍没有被借出
            {
                ...
                for(int i=0;i<10;i++)
                {
                    if(!strlen(users[user_index].lendBooksIsbn[i]))//如果该用户借书还没有满10本
                    {
                        strcpy(users[user_index].lendBooksIsbn[i],books[i].isbn);//把书籍的ISBN存入用户的借书记录中
...
void lendBooks() {
    ...
    for(int i=0;i<MAX_BOOKS_NUM;i++)//遍历books数组
    {
    ...
    while(scanf("%s", isbn))//获取图书isbn
    {
     ...
     GRAY;//灰色
        ...
        if(key == 'y' || key == 'Y')//如果确认归还，则将图书的借出日期改为空，并将用户的借书数量减1
        {
            ...
            for(int i=0;i<11;i++)//将用户的借书isbn数组中的此图书isbn改为空
            {
...
{//第一个是按照书籍ISBN查找，第二个是按照书名查找，第三个是搜索方式，为1时按照ISBN查找，为2时按照书名查找
    ...
    if(mode == 1)//按照ISBN查找
    {
        for(int i=0;i<=MAX_BOOKS_NUM;i++)//判断图书是否存在
        {
